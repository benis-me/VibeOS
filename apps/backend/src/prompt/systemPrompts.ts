...
\`\`\`
   ...
   - window-state (windowIds, state) — minimize, maximize or restore existing OS windows. windowIds is an array of real IDs (use WINDOW.windowId for this window), or "all" for app/system windows, excluding desktop widgets. state is "minimized", "maximized" or "normal". This changes the real window state without regenerating its contents. Use only when requested or appropriate to this interaction; business completion does not imply minimizing or closing.
